          ...
          code: `class OrderService
{
    ...
    private function calculateAmount(User $user, Product $product, int $qty): int
    {
        ...
        },
        {
          type: "callout",
          variant: "info",
          title: "private ロジックを単体テストしたくなったら",
          text: "上の calculateAmount は private なので、place() 越しにしかテストできません。割引計算そのものをピンポイントで単体テストしたい場合は、計算を public なメソッドや専用クラス(例: AmountCalculator)へ切り出すのが定石です。テストしやすさは『どこに何を置くか』という設計の指針にもなります(Day7のテストのレッスンで、切り出した計算を単体テストする例を見ます)。",
        },
        {
          type: "callout",
          variant: "why",
          title: "なぜ業務手順をサービス層に集約するのか",
          text: "複数モデルをまたぐ手順は、どのモデルにも属しません。コントローラに書けば再利用できず、無理に片方のモデルに押し込めば別のモデルへの依存が漏れます。中立なサービス層に置くことで、コントローラ・バッチ・APIのどこからでも同じ手順を呼べ、割引計算のような部分だけを単体テストできます。責務が『どこに何があるか』の形で整理されるのが利点です。なお、規模が大きくなり、データの取得・保存の詳細までサービスから切り離したくなった場合は、データアクセスを専用クラスに集約する [[repository-pattern]](リポジトリパターン)を併用する選択肢もあります。",
        },
...
class OrderService
{
    ...
    // =====================================================================
    {
      ...
      blocks: [
        ...
        {
          type: "paragraph",
          text: "この4性質をまとめて [[acid]] と呼びます。実務では特に原子性(全部成功か全部失敗か)を自分の言葉で説明できると強いです。",
        },
