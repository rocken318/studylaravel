...
import { getSqlAdjacentLessons } from "@/data/sql/curriculum";

const SQL_PREFACE =
  "私はプログラミング初学者で、SQLの考え方を学びながら、AIに手伝ってもらってデータベースのデータを読めるようになりたいです。";

export function SqlLessonView({ day, lesson }: { day: Day; lesson: Lesson }) {
  const { isLessonComplete, toggleLessonComplete, hydrated } = useProgress();
  const done = hydrated && isLessonComplete(lesson.id);
  const { prev, next } = getSqlAdjacentLessons(lesson.id);

  return (
    <article className="mx-auto max-w-reading">
      <nav className="mb-4 flex items-center gap-1.5 text-xs text-ink-faint">
        <Link href="/sql" className="hover:text-brand">
          初めてのSQL
        </Link>
        <span>/</span>
        <Link href={`/sql/${day.slug}`} className="hover:text-brand">
          Day {day.day}
        </Link>
        <span>/</span>
      ...
      <nav className="mt-10 grid grid-cols-2 gap-3 border-t border-base-border pt-6">
        <div>
          {prev && (
            <Link
              href={`/sql/${prev.day.slug}/${prev.lesson.slug}`}
        ...
        <div>
          {next && (
            <Link
              href={`/sql/${next.day.slug}/${next.lesson.slug}`}
