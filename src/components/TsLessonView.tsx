...
import { getTsAdjacentLessons } from "@/data/typescript/curriculum";

const TS_PREFACE =
  "私はプログラミング初学者で、TypeScriptを学びながらAIを組み込んだWebアプリ(AIプロダクト)を作れるようになりたいです。";

export function TsLessonView({ day, lesson }: { day: Day; lesson: Lesson }) {
  const { isLessonComplete, toggleLessonComplete, hydrated } = useProgress();
  const done = hydrated && isLessonComplete(lesson.id);
  const { prev, next } = getTsAdjacentLessons(lesson.id);
  ...
  return (
    <article className="mx-auto max-w-reading">
      <nav className="mb-4 flex items-center gap-1.5 text-xs text-ink-faint">
        ...
        <span>/</span>
        <Link href={`/typescript/${day.slug}`} className="hover:text-brand">
          Day {day.day}
        </Link>
        <span>/</span>
      ...
      <nav className="mt-10 grid grid-cols-2 gap-3 border-t border-base-border pt-6">
        <div>
          {prev && (
            <Link
              href={`/typescript/${prev.day.slug}/${prev.lesson.slug}`}
        ...
        <div>
          {next && (
            <Link
              href={`/typescript/${next.day.slug}/${next.lesson.slug}`}
