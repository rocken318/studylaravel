...
export function TsHome() {
  const { completedLessons, hydrated } = useProgress();

  const completedCount = useMemo(
    () =>
      hydrated
        ? tsAllLessons.filter((f) => completedLessons[f.lesson.id]).length
        : 0,
    [completedLessons, hydrated]
  );
  const pct = Math.round((completedCount / tsTotalLessonCount) * 100);
    ...
    <div className="mx-auto max-w-3xl">
      <header className="mb-8">
        ...
        <p className="mt-3 leading-relaxed text-ink-soft">
          7日で、TypeScriptの基礎から「AIをフロントから呼ぶ小さなAIプロダクト」の完成まで。
          <strong>型があると、AIが生成したコードの間違いに早く気づけます</strong>。
          実装はAIに任せ、型で守り、説明できる状態を目指します。登録不要・進捗はこのブラウザに保存されます。
        </p>
      </header>
      ...
      <div className="mb-8 rounded-2xl border border-base-border bg-base-surface p-5 shadow-sm">
        <div className="mb-2 flex items-center justify-between text-sm">
          ...
          <span className="text-ink-faint">
            {completedCount}/{tsTotalLessonCount} レッスン
          </span>
        </div>
