  ...
  return (
    <div className="mx-auto max-w-4xl">
      <header className="mb-6">
        <p className="mb-1 text-sm font-medium text-accent">HTML/CSS 用語集</p>
        <h1 className="text-2xl font-bold text-ink md:text-3xl">
          Webの用語を「読める・説明できる」状態にする
        </h1>
        <p className="mt-2 max-w-reading leading-relaxed text-ink-soft">
          レッスン本文中の用語をクリックしても、この解説が開きます。全{" "}
          {webGlossary.length} 語。
        </p>
      </header>
      ...
      <div className="mb-4">
        <input
          ...
          placeholder="用語を検索(例: 要素、flex、レスポンシブ)"
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        <Chip
          ...
          label={`すべて (${webGlossary.length})`}
        />
        {webGlossaryCategoryOrder.map((cat) => {
          const count = webGlossary.filter((t) => t.category === cat).length;
          return (
            <Chip
              key={cat}
              active={filter === cat}
