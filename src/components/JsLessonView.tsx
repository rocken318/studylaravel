...
import { getJsAdjacentLessons } from "@/data/javascript/curriculum";

const JS_PREFACE =
  "私はプログラミング初学者で、JavaScriptの基礎を学びながら、AIに手伝ってもらってWebページに動きをつけられるようになりたいです。";

export function JsLessonView({ day, lesson }: { day: Day; lesson: Lesson }) {
  const { isLessonComplete, toggleLessonComplete, hydrated } = useProgress();
  const done = hydrated && isLessonComplete(lesson.id);
  const { prev, next } = getJsAdjacentLessons(lesson.id);

  return (
    <article className="mx-auto max-w-reading">
      <nav className="mb-4 flex items-center gap-1.5 text-xs text-ink-faint">
        <Link href="/javascript" className="hover:text-brand">
          初めてのJavaScript
        </Link>
        <span>/</span>
        <Link href={`/javascript/${day.slug}`} className="hover:text-brand">
          Day {day.day}
        </Link>
        <span>/</span>
      ...
      <nav className="mt-10 grid grid-cols-2 gap-3 border-t border-base-border pt-6">
        <div>
          {prev && (
            <Link
              href={`/javascript/${prev.day.slug}/${prev.lesson.slug}`}
        ...
        <div>
          {next && (
            <Link
              href={`/javascript/${next.day.slug}/${next.lesson.slug}`}
