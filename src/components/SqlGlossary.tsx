  ...
  return (
    <div className="mx-auto max-w-4xl">
      <header className="mb-6">
        <p className="mb-1 text-sm font-medium text-accent">SQL 用語集</p>
        <h1 className="text-2xl font-bold text-ink md:text-3xl">
          SQLの用語を「読める・説明できる」状態にする
        </h1>
        <p className="mt-2 max-w-reading leading-relaxed text-ink-soft">
          レッスン本文中の用語をクリックしても、この解説が開きます。全{" "}
          {sqlGlossary.length} 語。
        </p>
      </header>
      ...
      <div className="mb-4">
        <input
          ...
          placeholder="用語を検索(例: SELECT、テーブル、JOIN、主キー)"
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        <Chip
          ...
          label={`すべて (${sqlGlossary.length})`}
        />
        {sqlGlossaryCategoryOrder.map((cat) => {
          const count = sqlGlossary.filter((t) => t.category === cat).length;
          return (
            <Chip
              key={cat}
              active={filter === cat}
