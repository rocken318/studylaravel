...
export function BasicsGlossary() {
  ...
  const filtered = useMemo(() => {
    ...
    return basicsGlossary.filter((t) => {
    ...
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mb-4">
        <input
          ...
          placeholder="用語を検索(例: 状態、API、デバッグ)"
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        <Chip
          ...
          label={`すべて (${basicsGlossary.length})`}
        />
        {basicsGlossaryCategoryOrder.map((cat) => {
          const count = basicsGlossary.filter((t) => t.category === cat).length;
          return (
            <Chip
              key={cat}
              active={filter === cat}
