...
import { getWebAdjacentLessons } from "@/data/web/curriculum";

const WEB_PREFACE =
  "私はプログラミング初学者で、HTML/CSSの基礎を学びながら、AIに手伝ってもらってWebページを作れるようになりたいです。";

export function WebLessonView({ day, lesson }: { day: Day; lesson: Lesson }) {
  const { isLessonComplete, toggleLessonComplete, hydrated } = useProgress();
  const done = hydrated && isLessonComplete(lesson.id);
  const { prev, next } = getWebAdjacentLessons(lesson.id);

  return (
    <article className="mx-auto max-w-reading">
      <nav className="mb-4 flex items-center gap-1.5 text-xs text-ink-faint">
        <Link href="/web" className="hover:text-brand">
          初めてのHTML/CSS
        </Link>
        <span>/</span>
        <Link href={`/web/${day.slug}`} className="hover:text-brand">
          Day {day.day}
        </Link>
        <span>/</span>
      ...
      <nav className="mt-10 grid grid-cols-2 gap-3 border-t border-base-border pt-6">
        <div>
          {prev && (
            <Link
              href={`/web/${prev.day.slug}/${prev.lesson.slug}`}
        ...
        <div>
          {next && (
            <Link
              href={`/web/${next.day.slug}/${next.lesson.slug}`}
