...
export function ReactHome() {
  ...
  const completedCount = useMemo(
    () =>
      hydrated
        ? reactAllLessons.filter((f) => completedLessons[f.lesson.id]).length
        : 0,
    [completedLessons, hydrated]
  );
  const pct = Math.round((completedCount / reactTotalLessonCount) * 100);
    ...
    <div className="mx-auto max-w-3xl">
      <header className="mb-8">
        ...
        <p className="mt-3 leading-relaxed text-ink-soft">
          7日で、Reactの考え方(コンポーネント・props・state・useEffect)から、
          データを取得して表示する小さなアプリの公開まで。
          <strong>実装はAIに任せても、Reactの仕組みを読める・直せる・説明できる</strong>状態を目指します。
          JavaScriptの基礎を終えていることが前提です。登録不要・進捗はこのブラウザに保存されます。
        </p>
      </header>
      ...
      <div className="mb-8 rounded-2xl border border-base-border bg-base-surface p-5 shadow-sm">
        <div className="mb-2 flex items-center justify-between text-sm">
          ...
          <span className="text-ink-faint">
            {completedCount}/{reactTotalLessonCount} レッスン
          </span>
        </div>
