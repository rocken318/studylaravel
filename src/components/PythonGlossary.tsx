...
export function PythonGlossary() {
  ...
  const filtered = useMemo(() => {
    ...
    return pythonGlossary.filter((t) => {
    ...
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mb-4">
        <input
          ...
          placeholder="用語を検索(例: 辞書、プロンプト、API)"
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        <Chip
          ...
          label={`すべて (${pythonGlossary.length})`}
        />
        {pythonGlossaryCategoryOrder.map((cat) => {
          const count = pythonGlossary.filter((t) => t.category === cat).length;
          return (
            <Chip
              key={cat}
              active={filter === cat}
