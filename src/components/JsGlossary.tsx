  ...
  return (
    <div className="mx-auto max-w-4xl">
      <header className="mb-6">
        <p className="mb-1 text-sm font-medium text-accent">JavaScript 用語集</p>
        <h1 className="text-2xl font-bold text-ink md:text-3xl">
          JavaScriptの用語を「読める・説明できる」状態にする
        </h1>
      ...
      <div className="mb-4">
        <input
          ...
          placeholder="用語を検索(例: 変数、配列、map、DOM)"
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        <Chip
          ...
          label={`すべて (${jsGlossary.length})`}
        />
        {jsGlossaryCategoryOrder.map((cat) => {
          const count = jsGlossary.filter((t) => t.category === cat).length;
          return (
            <Chip
              key={cat}
              active={filter === cat}
