...
const laravelNav = [
  ...
  { href: "/request-lifecycle", label: "リクエストライフサイクル", icon: "🔄" },
  { href: "/eloquent-relations", label: "Eloquentリレーション図解", icon: "🔗" },
  { href: "/auth-flow", label: "認証・認可フロー", icon: "🔐" },
  { href: "/deployment-guide", label: "本番デプロイ手順", icon: "🚀" },
  { href: "/interview", label: "想定Q&A集", icon: "🎤" },
  { href: "/laravel-interview", label: "実務で通用する 一覧表", icon: "📋" },
  { href: "/team-development", label: "チーム開発で大事なこと", icon: "🤝" },
...
const courses: Record<CourseKey, Course> = {
  basics: {
    ...
    nav: [
      ...
      { href: "/first-website", label: "はじめてのHP作り", icon: "🌐" },
  ...
  sql: {
    ...
    nav: [
      { href: "/sql", label: "コースホーム", icon: "🏠" },
      { href: "/sql/glossary", label: "SQL用語集", icon: "📘" },
    ],
  },
};
...
function activeCourseKey(pathname: string): CourseKey {
  ...
  if (pathname.startsWith("/portfolio")) return "basics";
  if (pathname.startsWith("/first-website")) return "basics";
