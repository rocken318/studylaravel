...
export function WebHome() {
  ...
  const completedCount = useMemo(
    () =>
      hydrated
        ? webAllLessons.filter((f) => completedLessons[f.lesson.id]).length
        : 0,
    [completedLessons, hydrated]
  );
  const pct = Math.round((completedCount / webTotalLessonCount) * 100);

  return (
    <div className="mx-auto max-w-3xl">
      <header className="mb-8">
        <p className="mb-1 text-sm font-bold text-accent">Webの土台シリーズ</p>
        <h1 className="text-2xl font-bold text-ink md:text-3xl">
          初めてのHTML/CSS — Webページの土台
        </h1>
        <p className="mt-3 leading-relaxed text-ink-soft">
          Webページが HTML(骨組み)・CSS(見た目)・JS(動き) の三層でできていることから、
          レイアウトとレスポンシブまでを4日で。
          <strong>自分では全部書けなくても、AIが出したコードを「読めて直せる」</strong>
          状態を目指します。登録不要・進捗はこのブラウザに保存されます。
        </p>
      </header>
      ...
      <div className="mb-8 rounded-2xl border border-base-border bg-base-surface p-5 shadow-sm">
        <div className="mb-2 flex items-center justify-between text-sm">
          ...
          <span className="text-ink-faint">
            {completedCount}/{webTotalLessonCount} レッスン
          </span>
        </div>
