...
import { getGitAdjacentLessons } from "@/data/git/curriculum";

const GIT_PREFACE =
  "私はプログラミング初学者で、Gitの考え方を学びながら、AIに手伝ってもらってコードを安全に管理できるようになりたいです。";

export function GitLessonView({ day, lesson }: { day: Day; lesson: Lesson }) {
  const { isLessonComplete, toggleLessonComplete, hydrated } = useProgress();
  const done = hydrated && isLessonComplete(lesson.id);
  const { prev, next } = getGitAdjacentLessons(lesson.id);

  return (
    <article className="mx-auto max-w-reading">
      <nav className="mb-4 flex items-center gap-1.5 text-xs text-ink-faint">
        <Link href="/git" className="hover:text-brand">
          初めてのGit
        </Link>
        <span>/</span>
        <Link href={`/git/${day.slug}`} className="hover:text-brand">
          Day {day.day}
        </Link>
        <span>/</span>
      ...
      <nav className="mt-10 grid grid-cols-2 gap-3 border-t border-base-border pt-6">
        <div>
          {prev && (
            <Link
              href={`/git/${prev.day.slug}/${prev.lesson.slug}`}
        ...
        <div>
          {next && (
            <Link
              href={`/git/${next.day.slug}/${next.lesson.slug}`}
