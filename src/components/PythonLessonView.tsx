...
import { getPythonAdjacentLessons } from "@/data/python/curriculum";

const PY_PREFACE =
  "私はプログラミング未経験からPythonを学んでいる初心者です。AIを使いこなして、自分でツールを作れるようになりたいです。";
...
export function PythonLessonView({
  ...
  const { prev, next } = getPythonAdjacentLessons(lesson.id);

  return (
    <article className="mx-auto max-w-reading">
      <nav className="mb-4 flex items-center gap-1.5 text-xs text-ink-faint">
        <Link href="/python" className="hover:text-brand">
          初めてのPython
        </Link>
        <span>/</span>
        <Link href={`/python/${day.slug}`} className="hover:text-brand">
          Day {day.day}
        </Link>
        <span>/</span>
      ...
      <nav className="mt-10 grid grid-cols-2 gap-3 border-t border-base-border pt-6">
        <div>
          {prev && (
            <Link
              href={`/python/${prev.day.slug}/${prev.lesson.slug}`}
        ...
        <div>
          {next && (
            <Link
              href={`/python/${next.day.slug}/${next.lesson.slug}`}
